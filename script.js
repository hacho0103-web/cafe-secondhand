(function(){
  var INSTA="https://www.instagram.com/groperseoul/";
  function fmt(p){if(typeof p==="string")return p;return p.toLocaleString("ko-KR")+"원";}
  function pad(n){return String(n).padStart(2,"0");}
  function counts(){
    var all=items.length,av=items.filter(function(i){return i.status==="available";}).length,sd=items.filter(function(i){return i.status==="sold";}).length;
  ...
  function render(filter){
    ...
    if(!list.length){grid.innerHTML="<p class=\"no-results\">해당 항목이 없습니다.</p>";return;}
    list.forEach(function(item,idx){
      ...
      var dotHtml=item.status==="sold"?"<div class=\"sold-dot\"><span class=\"sold-dot-text\">SOLD</span></div>":"";
      var tagsHtml="";
      if(item.qty>1) tagsHtml+="<span class=\"tag\">"+item.qty+"개</span><span class=\"tag-single\">낱개 구매 가능</span>";
      ...
      card.innerHTML=
        ...
        "<div class=\"item-body\">"+
          ...
          "<div class=\"item-price"+(item.status==="sold"?" is-sold":"")+"\">"+fmt(item.price)+"</div>"+
  ...
  function openLb(item){
    var lb=document.getElementById("lb"),c=document.getElementById("lb-content");
    var imgHtml=item.image
